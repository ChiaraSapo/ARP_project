        ...
        for (k = 0; k < number_of_iterations; ++k)
        {
            ...
            else
            {
                ...
                if (pilot == 0)
                {
                    ...
                    // By setting this cariable to 1, we make sure we will fall into this case only once
                    pilot = 1;
                    //start_signal = 0;
                    new_input = 1;
                }
                else if(pilot == 1)
                {
                    ...
                    new_input = 1;
                    new_input_1 = 0;
                    new_token_2 = new_token_1;
                    new_token_1 = received_token;
                    new_token = new_input_1 * sin(2 * pi * RF * DT) - new_input * sin(2 * pi * RF * DT) - new_token_2 + new_token_1 * 2 * cos(2 * pi * RF * DT);
                    pilot = 2;
                }
                else
                {
                ...
                printf("new_input_1 * sin(2 * pi * RF * DT): %f\n",new_input_1 * sin(2 * pi * RF * DT));
                printf("- new_input * sin(2 * pi * RF * DT): %f\n",(- new_input * sin(2 * pi * RF * DT)));
                printf("- new_token_2: %f\n",(- new_token_2));
                printf("new_token_1 * 2 * cos(2 * pi * RF * DT): %f\n",new_token_1 * 2 * cos(2 * pi * RF * DT));

                printf("new token: %f\n",new_token);
                printf("new token_1: %f\n",new_token_1);
                printf("new token_2: %f\n",new_token_2);
                printf("new_input: %f\n",new_input);
                printf("new_input_1: %f\n",new_input_1);
                printf("\n");
                /*
